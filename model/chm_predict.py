...
class CHM_Predict(object):

    @classmethod
    def initialize(cls, img_size, spatial_side):
        cls.img_size = img_size

        # cls.spatial_side = int(img_size / 8)
        cls.spatial_side = spatial_side
    ...
    def transfer_kps(cls, confidence_ts, src_kps, n_pts, normalized):
        ...
        print(confidence_ts.size())
        x
        if not normalized:
            src_kps = CHM_Predict.normalize_kps(src_kps)
        ...
        return CHM_Predict.unnormalize_kps(torch.stack(prd_kps))
